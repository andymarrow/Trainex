...
function TopFilters({ categories, selectedCategory, onCategorySelect, sortOptions, selectedSort, onSortChange, totalCourses }) {
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
      ...
      <div className="flex flex-grow overflow-x-auto hide-scrollbar py-2 md:py-0">
           {categories.map(category => (
              <motion.button
                key={category}
                onClick={() => onCategorySelect(category)}
                className={`flex-shrink-0 mr-3 last:mr-0 px-5 py-2 rounded-full text-sm font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 dark:focus:ring-cyan-400 whitespace-nowrap // Keep buttons from wrapping
